...
<head>
    ...
    <script>
        $(document).ready(function() {
            <?php 
                if (isset($_GET['id'])) {
                    echo "var url='http://localhost:3000/getAllFav/'+'".$_GET['id']."';";
                }
            ?>
            ...
            $.ajax({
                ...
                success: function(e) {
                    ...
                    for (var j = 0; j < e.length; j++) {
                        str = "";
                        var link = 'http://localhost:3000/getAllFavMv/' + e[j].mvid;
                        $.ajax({
                            type: 'GET',
                            url: link,
                            async: false,
        ...
        function checkfavMv(id, i) {
            var link = "http://localhost:3000/getFav/" + id + "/" + "<?php echo $_SESSION['id']; ?>";
            $.ajax({
                type: 'GET',
        ...
        function unFavMv(id) {
            var link = "http://localhost:3000/unFav/" + id + "/" + "<?php echo $_SESSION['id']; ?>";
            $.ajax({
                type: 'DELETE',
                contentType: 'application/json',
                url: link,
        ...
        function delfavMv(id) {
            if (confirm('Confirm???')) {
                var link = "http://localhost:3000/delMv/" + id;
                $.ajax({
                    type: 'DELETE',
                    url: link,
                    async: false,
        ...
        function favMv(id) {
            var link = "http://localhost:3000/favMv";
            var data = {};
            <?php 
